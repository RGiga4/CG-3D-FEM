...
main(int, char* argv[]) {
	...
	for (unsigned int i = 0; i < num_segments; ++i) {
		...
		// Set coordinates of the next edge of the polygon
		vertices[3 + i * 3 + 0] = radius * (float)cos(angle); // X
		vertices[3 + i * 3 + 1] = radius * (float)sin(angle); // Y
		...
		// Set indices of each triangle, with edges 0 and two neighboured edges of the polygon that approximates the circle
		indices[i * 3 + 0] = 0;
